...
class LinearRegression:
    ...
    def __costFunc(self):
        ...
        err = pred - self.Y
        cost = sum(err**2)/(2*m) +  self.lam * sum(self.theta[1:]**2)/(2*m)
        return(cost)
    ...
    def __gradientDescend(self, iter):
       ...
       """
        ...
        for i in range(0,iter):
            ...
            self.theta[0] = theta_temp[0] - self.alpha*(1.0/m)*sum(err*self.X[:,0])
            for j in range(1,n):
                val = theta_temp[j] - self.alpha*(1.0/m)*( sum(err*self.X[:,j])+ self.lam*m*theta_temp[j] )
                #print val
                self.theta[j] = val
...
def main():
    ...
    y = numpy.arange(0,10)+numpy.array([random.random() for r in range(0,10)])
